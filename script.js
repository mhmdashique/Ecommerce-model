...
document.addEventListener('click', (e) => {
    if (e.target.classList.contains('quick-add') || e.target.parentElement.classList.contains('quick-add')) {
        const btn = e.target.classList.contains('quick-add') ? e.target : e.target.parentElement;
...
document.addEventListener('DOMContentLoaded', () => {
    const animateElements = document.querySelectorAll('.product-card, .stat-item, .feature');
...
document.querySelector('.continue-shopping')?.addEventListener('click', closeCartAndContinue);
...
sendMessage.addEventListener('click', () => {
    ...
    if (message) {
        ...
        setTimeout(() => {
            addMessage('Thanks for your message! Our team will get back to you shortly.');
        }, 1000);
    }
});
